...
class Line:
    ...
    def process_message(self, message):
        ...
        if TOPIC_STATION == message_topic: # Set the conditional correctly to the stations Faust Table
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif TOPIC_ARRIVAL_TRAIN_PREFIX in message_topic: # Set the conditional to the arrival topic
            self._handle_arrival(message)
        elif TOPIC_TURNSTILE == message_topic: # Set the conditional to the KSQL Turnstile Summary Topic
